...
from toolbox.models import Settings_Model,QoutationItem
...
class SortPrice:
    def __init__(self):
        ...
        default_dir = initial_val_db(model=Settings_Model,category='path',tag='pick_sort_default_path',value= '/')
        ...
        with dpg.group():
            ...
            dpg.bind_item_theme('settings_sort',btnHovered_theme(colorHov=(104, 102, 97)))
            ...
            with dpg.group(tag = 'items_container'):
                with dpg.table(tag = 'item_table',clipper = True,parent  = 'items_container',width=985,height=500,row_background=True,scrollY = True,resizable = True, header_row = True,freeze_rows = 1):
                    if self.df is not None:
    ...
    def extract_data(self,sender,app_data,user_data):
        ...
        if 'Total_Price 4' in self.df.columns.to_list():
            ...
            qoutation_db = self.df[['Item_ID','Item_Name','T_Qty','Retail_Price','Price_4']]
           
            for item in qoutation_db.values.tolist():
                add_item = QoutationItem(
                    name = item[0],
                    description = item[1],
                    t_qty = item[2],
                    retail_price = item[3],
