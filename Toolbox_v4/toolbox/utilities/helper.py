...
from toolbox.models import AtdTables_Model,CashCount_Model,Personal
...
def initial_val_db(model:Any,category:str, tag:str, value:str):
    path = session.scalars(select(model).where(model.tag == tag)).first()
